...
abstract_gate_state_t::abstract_gate_state_t(const string &state_name,
...
abstract_filter_state_t::abstract_filter_state_t(const string &state_name,
                                                 const string &retry_state_name,
                                                 const string &thru_state_name,
                                                 machine_t *owner)
...
state_epoch_t::state_epoch_t(machine_t *owner)
    : abstract_gate_state_t("EPOCH", "WAITING", owner)
{
    is_open = access(lost(), F_OK) != 0;
}

state_waiting_t::state_waiting_t(machine_t *owner)
    : abstract_gate_state_t("WAITING", "NEW", owner)
{
    is_open = false;
}
...
void state_new_t::enter(event_t *e)
{
    ...
    abstract_state_t *next_state
        = machine->state_aborted; // TODO: make a new state "FAILED" for this case

    if (e->flags & EventFlags::In_Dialog) {
        ticker_t t_now = machine->transition_start_time;

        bool restore = true;
        if (e->last_triggered.is_valid()
            && t_now - e->last_triggered > machine->dialog_discard_threshold)
            restore = false;
...
void state_scheduler_t::enter(event_t *e)
{
    ...
    if (e->has_ticker())
        ...
        log_debug("now=%ld e->trigger=%ld diff=%ld",
                  time(NULL),
                  e->trigger.value(),
                  e->trigger.value() - time(NULL));
...
state_queued_t::state_queued_t(machine_t *owner)
    : abstract_io_state_t("QUEUED", owner)
{
    pause_x = 0;
...
void state_queued_t::alarm_timeout()
{
    ...
    log_debug("time_now=%ld queue.begin=%ld",
              time_now.value(),
              queue.begin() != queue.end() ? queue.begin()->first.value() : (time_t) -1);
    typedef set<event_pair>::iterator iterator;
    bool event_found = false;
    for (iterator it = queue.begin(); it != queue.end() && it->first <= time_now;
         ++it, event_found = true)
...
void state_queued_t::timer_start()
{
    ...
    nanotime_t time_to_wait = nanotime_t(queue.begin()->first.value(), 0)
                              - nanotime_t::systime_now();
    bool no_sleep = false;
    if (time_to_wait < 0) {
...
void state_queued_t::filter_closed(abstract_filter_state_t *f_st)
{
    ...
    for (iterator it = queue.begin(); it != queue.end(); ++it) {
        ...
        event_found = true;
        log_debug("event_found=%d", event_found);
        log_debug("event [%u] found in state '%s', requesting staet '%s'",
...
void state_due_t::enter(event_t *e)
{
    ...
    log_notice("due event [%d]: e->trigger=%ld, now=%ld, next_state='%s'",
               e->cookie.value(),
               e->trigger.value(),
               machine->transition_started().value(),
               next_state->name());
    machine->request_state(e, next_state);
}
...
static ticker_t recur_irregular_day(const broken_down_t &day,
...
static ticker_t recur_regular_day(const broken_down_t &day,
                                  const recurrence_pattern_t *p,
                                  time_t threshold)
{
    log_debug("day=%s, p->mins=0x%llx, threshold=%lld",
              day.str().c_str(),
              p->mins,
              (long long) threshold);
    broken_down_t d = day;
...
ticker_t state_recurred_t::apply_pattern(const broken_down_t &start,
                                         int wday,
                                         const recurrence_pattern_t *p,
                                         qlonglong offset)
{
    ...
    for (bool today_flag = true; day.find_a_good_day(p, wday, today_flag, nxt_year);
         today_flag = false) {
        if (not today_flag)
            day.hour = day.minute = 0;
        ticker_t t = (day.is_a_regular_day() ? recur_regular_day
                                             : recur_irregular_day)(day, p, threshold);
        if (t.is_valid())
            return t;
    }
    return ticker_t();
...
state_button_t::state_button_t(machine_t *owner, signed n)
    : abstract_state_t(button_state_name(n), owner)
{
    no = n;
...
void state_button_t::enter(event_t *e)
{
    ...
    if (e->tsz_max) {
        log_debug("processing restricted timeout snooze event: cookie=%d, max=%d, count=%d",
                  e->cookie.value(),
                  e->tsz_max,
                  e->tsz_counter);
        ...
        log_debug("processed: cookie=%d, max=%d, count=%d, snooze=%d",
                  e->cookie.value(),
                  e->tsz_max,
                  e->tsz_counter,
                  snooze_length);
