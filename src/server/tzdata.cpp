...
int tzdata::by_country(const string &alpha2, enum tzdata::zone_type type, set<olson *> &out)
{
    ...
    int counter = 0;
    int threshold = type == tzdata::Main_Zones   ? 1
                    : type == tzdata::Real_Zones ? 2
                                                 : 3; // all by default
    const map<string, int> &zone_to_level = it->second;

    for (map<string, int>::const_iterator jt = zone_to_level.begin(); jt != zone_to_level.end();
         ++jt)
...
void tzdata::init(const string &default_tz)
{
    ...
    iodata::record *C = open_database("/usr/share/tzdata-timed/zones-by-country.data",
                                      "zones_by_country_t");
    ...
    const iodata::array *b
        = B->get("list")->arr(); // TODO: rename list->tz_single (here and in tzdata script)
    ...
    for (unsigned i = 0; i < c->size(); ++i) {
        ...
        for (int important = 1; important <= 2; ++important) {
            ...
            for (unsigned j = 0; j < list->size(); ++j) {
                ...
                int i_value = (important == 1 and j == 0)
                                  ? 0
                                  : important; // the very first is the capital
