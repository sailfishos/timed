...
void map_q_to_std(const QMap<QString, QString> &from, map<string, string> &to)
{
    for (QMap<QString, QString>::const_iterator it = from.begin(); it != from.end(); ++it) {
        ...
        if (old != to.end())
            log_warning("Duplicate key: '%s'=>'%s', using new value '%s'",
                        k.c_str(),
                        old->second.c_str(),
                        v.c_str());
        to[k] = v;
...
void map_std_to_q(const map<string, string> &from, QMap<QString, QString> &to)
{
    for (map<string, string>::const_iterator it = from.begin(); it != from.end(); ++it) {
        ...
        if (k.isEmpty() || v.isEmpty()) {
            log_warning("Ignoring empty key or value: '%s'=>'%s'",
                        it->first.c_str(),
                        it->second.c_str());
            continue;
        }
        ...
        if (old != to.end())
            log_warning("Duplicate key: '%s'=>'%s', using new value '%s'",
                        it->first.c_str(),
                        string_q_to_std(old.value()).c_str(),
                        it->second.c_str());
        to.insert(k, v);
    }
}
...
string str_iso8601(time_t t) // 2008-05-11T15:30:00Z
{
    ...
    if (gmtime_r(&t, &tm) != &tm)
        ...
        return str_printf("%04d-%02d-%02dT%02d:%02d:%02dZ",
                          tm.tm_year + 1900,
                          tm.tm_mon + 1,
                          tm.tm_mday,
                          tm.tm_hour,
