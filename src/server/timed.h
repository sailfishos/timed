...
public:
    ...
    cookie_t add_event(cookie_t remove,
                       const Maemo::Timed::event_io_t &event,
                       const QDBusMessage &message);
    void add_events(const Maemo::Timed::event_list_io_t &events,
                    QList<QVariant> &res,
                    const QDBusMessage &message);
    ...
    {
        am->cancel_events(cookies, failed);
    }
...
private:
    ...
    void load_rc();
    void load_settings();
    bool permissions_helper(const QString &label,
