...
bool broken_down_t::same_struct_tm(const struct tm *tm) const
{
    ...
    if (!is_valid())
        return false;
#endif
    return (int) year == tm->tm_year + 1900 && (int) month == tm->tm_mon + 1
           && (int) day == tm->tm_mday && (int) hour == tm->tm_hour && (int) minute == tm->tm_min;
}

bool broken_down_t::find_a_good_day(const recurrence_pattern_t *p,
                                    int &wday,
                                    bool &today,
                                    unsigned max_year)
{
...
bool broken_down_t::find_a_good_minute_with_increment(const recurrence_pattern_t *p,
                                                      bool increment_flag)
{
    if (increment_flag) {
