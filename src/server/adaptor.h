...
public slots:
    ...
    {
        log_notice("DBUS::com.nokia.time.add_event(APP='%s') by %s",
                   x.attr.txt["APPLICATION"].QC,
                   PEER);
    ...
    void add_events(const Maemo::Timed::event_list_io_t &lst,
                    const QDBusMessage &message,
                    QList<QVariant> &res)
    {
    ...
    void get_event(uint cookie, const QDBusMessage &message, Maemo::Timed::event_io_t &res)
    {
        ...
        if (!timed->get_event(cookie_t(cookie), res)) {
            QDBusMessage errorReply = message.createErrorReply(QDBusError::InvalidArgs,
                                                               "Cookie is invalid or not found");
            Maemo::Timed::bus().send(errorReply);
        }
    }

    void get_events(Q_List_uint cookies,
                    const QDBusMessage &message,
                    Maemo::Timed::event_list_io_t &res)
    {
        ...
        if (!timed->get_events(cookies, res)) {
            QDBusMessage errorReply
                = message.createErrorReply(QDBusError::InvalidArgs,
                                           "One of cookies is invalid or not found");
            Maemo::Timed::bus().send(errorReply);
        }
    }
    ...
    {
        log_notice("DBUS::com.nokia.time.replace_event(APP='%s', cookie=%u) by %s",
                   x.attr.txt["APPLICATION"].QC,
                   old,
                   PEER);
    ...
    {
        Q_UNUSED(message);
        log_notice("DBUS::com.nokia.time.dialog_response(cookie=%u, value=%d) by %s",
                   cookie,
                   value,
                   PEER);
    ...
    void query(const QMap<QString, QVariant> &words,
               const QDBusMessage &message,
               QList<QVariant> &res)
    {
        Q_UNUSED(message);
    ...
    void get_attributes_by_cookies(const Q_List_uint &cookies,
                                   const QDBusMessage &message,
                                   Q_Map_uint_String_String &a)
    {
        Q_UNUSED(message);
    ...
    void get_cookies_by_attributes(const Q_Map_String_String &words,
                                   const QDBusMessage &message,
                                   Q_List_uint &res)
    {
        Q_UNUSED(message);
    ...
    {
        Q_UNUSED(message);
        log_notice("DBUS::com.nokia.time.set_app_snooze(name='%s',value=%d) by %s",
    ...
    {
        Q_UNUSED(message);
        log_notice("DBUS::com.nokia.time.get_default_snooze(name='%s') by %s",
                   name.toStdString().c_str(),
                   PEER);
    ...
    {
        Q_UNUSED(message);
        log_notice("DBUS::com.nokia.time.remove_app_snooze(name='%s') by %s",
                   name.toStdString().c_str(),
                   PEER);
...
#if OFONO
    bool fake_csd_time_signal(const QString &modem,
                              const QString &mcc,
                              const QString &mnc,
                              int offset,
                              int time,
                              ...
                              int nano_seconds)
    {
        log_notice("(fake_csd_time_signal) mcc='%s' mnc='%s' offset=%d time=%d dst=%d seconds=%d "
                   "nano_seconds=%d",
    ...
    bool fake_csd_time_signal_now(
        const QString &modem, const QString &mcc, const QString &mnc, int offset, int time, int dst)
    {
        log_notice(
            "(fake_csd_time_signal_now) modem='%s' mcc='%s' mnc='%s' offset=%d time=%d dst=%d",
    ...
    {
        log_notice("FAKE_CSD::csd_operator_s {modem='%s', mcc='%s', mnc='%s'}",
                   modem.toStdString().c_str(),
                   mcc.toStdString().c_str(),
                   mnc.toStdString().c_str());
