...
void Timed::init_device_mode()
{
    ...
    QObject::connect(dsme_mode_handler,
                     &dsme_mode_t::mode_is_changing,
                     this,
                     &Timed::dsme_mode_is_changing);
    QObject::connect(dsme_mode_handler,
                     &dsme_mode_t::mode_reported,
                     this,
                     &Timed::dsme_mode_reported);
    {
    ...
    QDBusConnection::systemBus().callWithCallback(QDBusMessage::createMethodCall(devicelock_service,
                                                                                 devicelock_path,
                                                                                 devicelock_interface,
                                                                                 "state"),
                                                  this,
                                                  SLOT(devicelock_state_changed(int)));
    QDBusConnection::systemBus().connect(devicelock_service,
                                         devicelock_path,
                                         devicelock_interface,
                                         "stateChanged",
                                         this,
    ...
    bool res1 = QDBusConnection::systemBus().connect("",
                                                     startup_path,
                                                     startup_iface,
                                                     "desktop_visible",
                                                     this,
                                                     desktop_visible_slot);
    bool res2 = QDBusConnection::systemBus()
                    .connect("", startup_path, startup_iface, "init_done", this, init_done_slot);
    if (not res1)
                               ...
                               const QString &group) const
{
    ...
    if (directory.isEmpty()) {
        ...
        log_critical("%s: directory not configured", qUtf8Printable(label));
               ...
               && !sailfish_access_control_hasgroup(geteuid(), qUtf8Printable(group))) {
        /* It is expected that some users do not have access to shared alarms */
        log_debug("%s: uid %d - not in %s group",
...
{
    return permissions_helper("private_events_storage", private_data_directory, true, QString());
}
...
#define apply_cust(key, val) \
    do { \
        if (tree->get(key)->value() < 0) \
            tree->add(key, val); \
    } while (false)
...
void Timed::init_create_event_machine()
{
    ...
    QObject::connect(short_save_threshold_timer,
                     SIGNAL(timeout()),
                     this,
                     SLOT(queue_threshold_timeout()));
    QObject::connect(long_save_threshold_timer,
                     SIGNAL(timeout()),
                     this,
                     SLOT(queue_threshold_timeout()));

    QObject::connect(am,
                     SIGNAL(child_created(unsigned, int)),
                     this,
                     SLOT(register_child(unsigned, int)));
    ...
    QObject::connect(am,
                     SIGNAL(next_bootup_event(int, int)),
                     this,
                     SIGNAL(next_bootup_event(int, int)));
    voland_watcher = NULL;
    ...
    QObject::connect(am,
                     SIGNAL(alarm_trigger(QMap<QString, QVariant>)),
                     this,
                     SLOT(set_alarm_trigger(QMap<QString, QVariant>)));
}
...
void Timed::start_voland_watcher()
{
    ...
    QObject::connect(voland_watcher,
                     SIGNAL(serviceOwnerChanged(QString, QString, QString)),
                     this,
                     SLOT(system_owner_changed(QString, QString, QString)));
...
void Timed::init_main_interface_object()
{
    ...
    if (conn.registerObject(path, this))
        ...
        log_critical("remote methods not available; failed to register dbus object: %s",
                     Maemo::Timed::bus().lastError().message().toStdString().c_str());
    // XXX:
...
void Timed::init_main_interface_dbus_name()
{
    ...
    if (Maemo::Timed::bus().registerService(Maemo::Timed::service()))
        log_info("service name '%s' registered on bus '%s'",
                 Maemo::Timed::service(),
                 conn_name.c_str());
    else {
        const string msg = Maemo::Timed::bus().lastError().message().toStdString();
        log_critical("can't register service '%s' on bus '%s': '%s'",
                     Maemo::Timed::service(),
                     conn_name.c_str(),
                     msg.c_str());
...
void Timed::init_cellular_services()
{
    ...
    bool res1 = QObject::connect(csd,
                                 SIGNAL(csd_cellular_time(const cellular_time_t &)),
                                 settings,
                                 SLOT(cellular_time_slot(const cellular_time_t &)));
    bool res2 = QObject::connect(csd,
                                 SIGNAL(csd_cellular_offset(const cellular_offset_t &)),
                                 tz_oracle,
                                 SLOT(cellular_offset(const cellular_offset_t &)));
    bool res3 = QObject::connect(csd,
                                 SIGNAL(csd_cellular_operator(const cellular_operator_t &,
                                                              const QString &)),
                                 tz_oracle,
                                 SLOT(cellular_operator(const cellular_operator_t &,
                                                        const QString &)));
    bool res4 = QObject::connect(tz_oracle,
                                 SIGNAL(cellular_zone_detected(olson *, suggestion_t, bool)),
                                 settings,
                                 SLOT(cellular_zone_slot(olson *, suggestion_t, bool)));
...
cookie_t Timed::add_event(cookie_t remove,
                          const Maemo::Timed::event_io_t &x,
...
void Timed::add_events(const Maemo::Timed::event_list_io_t &events,
                       QList<QVariant> &res,
                       const QDBusMessage &message)
{
    if (events.ee.size() == 0) {
    ...
    if (test != events.ee[0].attr.txt.end())
        log_debug("TEST list of %d events: '%s'",
                  events.ee.size(),
                  test.value().toStdString().c_str());
...
void Timed::system_owner_changed(const QString &name,
...
#define __qstr(a) (a.isEmpty() ? "<empty>" : a.toStdString().c_str())
    if (name_match)
        log_info("Service %s owner changed from %s to %s",
                 __qstr(name),
                 __qstr(oldowner),
                 __qstr(newowner));
    else
        log_error("expecing notification about '%s' got about '%s'",
...
void Timed::save_event_queue()
{
    // Private data
    if (permissions_private_events()) {
        iodata::record *queue = am->save(false,
                                         machine_t::PrivateEvents); // false = full queue, not backup
    ...
    if (permissions_shared_events()) {
        iodata::record *queue = am->save(false,
                                         machine_t::SharedEvents); // false = full queue, not backup
...
void Timed::save_settings()
{
    ...
    if (access(qUtf8Printable(shared_settings_path), R_OK) == 0) {
        if (unlink(qUtf8Printable(private_settings_path)) == 0) {
            ...
            log_error("%s: can't remove file: %m", qUtf8Printable(private_settings_path));
        }

        QString backup = private_settings_path + ".bak";
        if (unlink(qUtf8Printable(backup)) == 0) {
            log_warning("%s: stale configuration file removed", qUtf8Printable(backup));
        } else if (errno != ENOENT) {
            log_error("%s: can't remove file: %m", qUtf8Printable(backup));
        }
    }
}
...
void Timed::init_kernel_notification()
{
    ...
    QObject::connect(notificator,
                     SIGNAL(system_time_changed(const nanotime_t &)),
                     this,
                     SLOT(kernel_notification(const nanotime_t &)));
...
void Timed::init_first_boot_hwclock_time_adjustment_check()
{
    ...
    if (QDate::currentDate().year() < 2013) {
        log_info("first boot, updating old date from year %d to 01/01/2013",
