...
struct recurrence_pattern_t
{
    ...
    static iodata::bit_codec *codec, *mins_codec, *hour_codec, *mday_codec, *wday_codec,
        *mons_codec;
...
struct attribute_t
{
    ...
    {
        const_iterator it = txt.find(key);
        return it == txt.end() ? string() : it->second;
    }
...
{
    const event_t *event;
    action_comparison_t(const event_t *e)
        : event(e)
    {}
...
struct event_exception : public std::exception
{
    ...
    event_exception(const string &msg)
        : message(msg)
        , pid_value(getpid())
    {}
