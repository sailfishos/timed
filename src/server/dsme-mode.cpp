    ...
    : QObject(parent)
{
    ...
    bool res = QDBusConnection::systemBus().connect(dsme_service,
                                                    dsme_sig_path,
                                                    dsme_sig_interface,
                                                    dsme_state_change_ind,
                                                    this,
                                                    SLOT(dsme_state_signalled(const QString &)));
    if (not res)
...
void dsme_mode_t::init_request()
{
    ...
    QObject::connect(request_watcher,
                     SIGNAL(finished(QDBusPendingCallWatcher *)),
                     this,
                     SLOT(request_finished(QDBusPendingCallWatcher *)));
}
