...
machine_t::machine_t(const Timed *daemon)
    : timed(daemon)
{
    log_debug();
    // T = transition state
    ...
    // -->NEW loaded as new
    // -->DUE loaded as due
    abstract_state_t *S[] = {state_start = new state_start_t(this),         // T
    ...
    QObject::connect(state_queued,
                     SIGNAL(sleep()),
                     state_dlg_cntr,
                     SLOT(open()),
                     Qt::QueuedConnection);
    QObject::connect(state_dlg_wait,
                     SIGNAL(opened()),
                     state_dlg_cntr,
                     SLOT(open()),
                     Qt::QueuedConnection);

    QObject::connect(state_flt_alrm,
                     SIGNAL(closed(abstract_filter_state_t *)),
                     state_queued,
                     SLOT(filter_closed(abstract_filter_state_t *)));
    QObject::connect(state_flt_user,
                     SIGNAL(closed(abstract_filter_state_t *)),
                     state_queued,
                     SLOT(filter_closed(abstract_filter_state_t *)));
...
machine_t::~machine_t()
{
    ...
    for (map<uint32_t, abstract_cluster_t *>::iterator it = clusters.begin(); it != clusters.end();
         ++it)
        delete it->second;
...
string machine_t::s_states()
{
    ...
    for (map<string, set<event_t *>>::const_iterator it = s2e.begin(); it != s2e.end(); ++it)
        for (set<event_t *>::const_iterator q = it->second.begin(); q != it->second.end(); ++q)
            os << (q == it->second.begin() ? string(it == s2e.begin() ? "" : " ") + it->first + ":"
                                           : ",")
               << (*q)->cookie.value();
...
{
    ostringstream os;
    for (deque<pair<event_t *, abstract_state_t *>>::const_iterator it = transition_queue.begin();
         it != transition_queue.end();
         ++it) {
...
#define state_name(p) (p ? p->name() : "null")
        ...
        if (not is_event_registered(e)) {
            log_error("requested to move destroyed event %p to state '%s'",
                      e,
                      state_name(new_state));
            continue;
        }
        abstract_state_t *old_state = e->get_state();
        log_notice("State transition %d:'%s'->'%s'",
                   e->cookie.value(),
...
void machine_t::send_bootup_signal()
{
    ...
    if ((signalled_bootup < 0 || signalled_bootup != next_bootup)
        ...
        emit next_bootup_event(signalled_bootup = next_bootup,
                               signalled_non_boot_event = next_non_boot_event);
    }
    log_debug();
}
...
void machine_t::reshuffle_queue(const nanotime_t &back)
{
    ...
    for (abstract_io_state_t **p = from, *state; (state = *p); ++p) {
        for (set<event_t *>::iterator it = state->events.begin(); it != state->events.end(); ++it) {
            ...
            bool trigger_when_settings_changed = e->flags
                                                 & EventFlags::Trigger_When_Settings_Changed;
            ...
            if (e->flags
                & EventFlags::Empty_Recurring) // TODO: rewrite recurrence calculation and get rid of "Empty_Recurring"
                e->invalidate_t();
...
cookie_t machine_t::add_event(const Maemo::Timed::event_io_t *eio,
                              bool process_queue,
                              const credentials_t *creds,
                              const QDBusMessage *p_message)
{
...
void machine_t::add_events(const Maemo::Timed::event_list_io_t &lst,
                           QList<QVariant> &res,
...
void machine_t::get_attributes_by_cookies(const QList<uint> &cookies,
                                          QMap<uint, QMap<QString, QString>> &a)
{
...
bool machine_t::dialog_response(cookie_t c, int value)
{
    ...
    int b = e->b_attr.size();
    if (value > b) {
        log_error("Invalid value in respond: event [%d], value=%d, number of buttons: %d",
                  c.value(),
                  value,
                  b);
        return false;
...
{
    flags = suppressed ? (flags | MachineFlags::Suppress_Alarms)
                       : (flags & ~MachineFlags::Suppress_Alarms);
}
...
void request_watcher_t::watch(const QDBusPendingCall &async_call)
{
    ...
    QObject::connect(w,
                     ...
                     this,
                     SLOT(call_returned(QDBusPendingCallWatcher *)));
}
