...
QDBusArgument &operator<<(QDBusArgument &out,
                          const Maemo::Timed::WallClock::wall_settings_pimple_t &x)
{
    qdbusargument_structrure_wrapper o(out);
    return out << x.opcodes << x.time_at_zero << x.offset << x.zone;
}

const QDBusArgument &operator>>(const QDBusArgument &in,
                                Maemo::Timed::WallClock::wall_settings_pimple_t &x)
{
