...
const char *OfonoConstants::OFONO_NETWORKREGISTRATION_INTERFACE
    = "org.fakeofono.NetworkRegistration";
...
void ut_networktime::test_networktimeinfo()
{
    ...
    networkTimeInfo = NetworkTimeInfo(dateTime,
                                      daylightAdjustment,
                                      offsetFromUtc,
                                      timestampSeconds,
...
bool ut_networktime::verifyNetworkTimeInfo(const NetworkTimeInfo timeInfo,
                                           const qlonglong utc,
                                           ...
                                           const QString modem)
{
    return timeInfo.isValid() && timeInfo.dateTime() == QDateTime::fromMSecsSinceEpoch(utc * 1000)
           && timeInfo.timestamp()->tv_sec == received && timeInfo.timestamp()->tv_nsec == 0
           && timeInfo.offsetFromUtc() == offsetFromUtc
           && timeInfo.daylightAdjustment() == (int) dst && timeInfo.mcc().compare(mcc) == 0
           && timeInfo.mnc().compare(mnc) == 0 && timeInfo.modem().compare(modem) == 0;
}
...
void ut_networktime::test_networktime()
{
    ...
    fakeOfono
        .emulateNetworkTimeChange(m1utc, m1received, m1offsetFromUtc, m1dst, m1mcc, m1mnc, modem1);
    QTest::qWait(500); // Give some time to DBus messaging

    QVERIFY(networkTime.isValid());
    QVERIFY(spy.count() == 1);
    QVERIFY2(verifyNetworkTimeInfo(networkTime.timeInfo(),
                                   m1utc,
                                   m1received,
                                   m1offsetFromUtc,
                                   m1dst,
    ...
    QVERIFY2(verifyNetworkTimeInfo(networkTime.timeInfo(modem1),
                                   m1utc,
                                   m1received,
                                   m1offsetFromUtc,
                                   m1dst,
    ...
    QVERIFY2(verifyNetworkTimeInfo(
                 timeInfo, m1utc, m1received, m1offsetFromUtc, m1dst, m1mcc, m1mnc, modem1),
             "NetworkTime::timeInfoQueryCompleted() signal provided an erronoeus NetworkTimeInfo "
             "object");
    ...
    fakeOfono
        .emulateNetworkTimeChange(m2utc, m2received, m2offsetFromUtc, m2dst, m2mcc, m2mnc, modem2);
    QTest::qWait(500); // Give some time to DBus messaging

    // we should now have a valid network time from the second modem
    QVERIFY(networkTime.isValid(modem2));
    QCOMPARE(spy.count(), currSpyCount + 1);
    QVERIFY2(verifyNetworkTimeInfo(networkTime.timeInfo(modem2),
                                   m2utc,
                                   m2received,
                                   m2offsetFromUtc,
                                   m2dst,
