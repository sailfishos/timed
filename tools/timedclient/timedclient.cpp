...
static const char *const bool_y[] = {"true", "yes", "on", "y", "t", 0};

/** Strings that qualify as bool false */
static const char *const bool_n[] = {"false", "no", "off", "n", "f", 0};
...
static bool parse_bool(const char *str)
{
    for (int i = 0; bool_y[i]; ++i) {
        if (!strcmp(bool_y[i], str))
            return true;
    }
    for (int i = 0; bool_n[i]; ++i) {
        if (!strcmp(bool_n[i], str))
            return false;
    }
...
static bool action_get_sys_button(const Maemo::Timed::Event::Action &act, int n)
{
    return act.whenSysButtons().indexOf(n) != -1;
}

static void action_set_sys_button(Maemo::Timed::Event &eve, Maemo::Timed::Event::Action &act, int n)
{
    (void) eve; // event ref not needed for sys buttons
    act.whenSysButton(n);
}

static void action_clr_sys_button(Maemo::Timed::Event &eve, Maemo::Timed::Event::Action &act, int n)
{
    (void) eve; // event ref not needed for sys buttons
...
static bool action_get_usr_button(const Maemo::Timed::Event::Action &act, int n)
{
    return act.whenButtons().indexOf(n) != -1;
}

static void action_set_usr_button(Maemo::Timed::Event &eve, Maemo::Timed::Event::Action &act, int n)
{
    Maemo::Timed::Event::Button &button = eve.button(n);
    act.whenButton(button);
}

static void action_clr_usr_button(Maemo::Timed::Event &eve, Maemo::Timed::Event::Action &act, int n)
{
    Maemo::Timed::Event::Button &button = eve.button(n);
                         ...
                         Maemo::Timed::WallClock::TimezoneSource src)
{
    ...
    printf("%-20s %s\n", TimezoneSource_repr(src), tz.toUtf8().constData());
}
                          ...
                          Maemo::Timed::WallClock::UtcSource src)
{
    ...
    if (t < 0) {
        printf("%-20s %s\n", UtcSource_repr(src), "N/A");
    } else {
        printf("%-20s %ld\n", UtcSource_repr(src), (long) t);
    }
}
                             ...
                             Maemo::Timed::WallClock::OffsetSource src)
{
    ...
    if (t < 0) {
        printf("%-20s %s\n", OffsetSource_repr(src), "N/A");
    } else {
        printf("%-20s %d\n", OffsetSource_repr(src), t);
    }
}
...
#define Xs(v) printf("%-20s %s\n", #v, info.v().toUtf8().constData())
        ...
        printf("%-20s %s\n", "utcSource", UtcSource_repr(info.utcSource()));

        printf("%-20s %s\n", "timezoneSource", TimezoneSource_repr(info.timezoneSource()));

        printf("%-20s %s\n", "offsetSource", OffsetSource_repr(info.offsetSource()));
...
static const struct option OPT_L[] = {{"help", 0, 0, 'h'},
                                      {"usage", 0, 0, 'h'},
...
static const char OPT_S[] = "h"  // --help
                            "l"  // --list
                            "i"  // --info
                            "p"  // --ping
                            "s:" // --search=<args>
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        case 'g':
            ...
            break;
        case 'b':
            do_add_button(optarg);
            break;
        case 'a':
            do_add_action(optarg);
            break;
        case 'r':
            do_add_recurrence(optarg);
            break;
        case 'e':
            do_add_event(optarg);
            break;
        case 'c':
            do_cancel_event(optarg);
            break;
        case 'C':
            do_cancel_events();
            break;
        case 'd':
            do_dismiss_event(optarg);
            break;
        case 001:
            do_set_enabled(optarg);
            break;
        case 002:
            do_get_enabled();
            break;
        case 003:
            do_set_snooze(optarg);
            break;
        case 004:
            do_get_snooze();
            break;
        case 005:
            ...
            break;
        case 006:
            do_set_app_snooze(optarg);
            break;
        case 007:
            do_get_app_snooze(optarg);
            break;
        case 010:
            do_get_info();
            break;
